...
public:
	...
	void out_eng()
	{
		std::cout << "Цилиндры: " << cylinders << " Объем: " << capacity << " Мощность: " << power << " Электро: " << nalichie << std::endl;
...
public:
	...
	std::string GetInfo()
	{
		return "Двигатель: кол-во целинтров = " + std::to_string(a_cylinders) + " объем = " + std::to_string(a_capacity);
	}
	void Read()
	{
		puts("Введите кол-во целиндров в двигателе:");
...
public:
	...
	std::string GetInfoE()
	{
		return "Электродвигатель: батарея  " + std::to_string(battery);
	}
